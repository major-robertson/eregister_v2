...
/**
 * Kanban column for the Formations admin board. Currently a 1:1 mapping
 * with FormApplicationStateAdminStatus, but kept as a separate enum so the
 * UI grouping stays decoupled from the underlying status (matches the
 * lien / sales-tax KanbanColumn pattern).
 */
enum FormationKanbanColumn: string
{
    case New = 'new';
    ...
    /**
     * The default board hides AwaitingClient (parked on customer, not active
     * admin work) and Approved (terminal). The /board/all view uses cases()
     * to show everything.
     *
    ...
    /**
     * Map a FormApplicationState to its kanban column based on the current
     * admin status.
     */

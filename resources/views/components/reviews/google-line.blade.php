{{--
    ...
    nothing if the rating is missing. The review count is left out on
    purpose while it is small.
--}}
@props(['dark' => false])
...
<p {{ $attributes->class(['flex flex-wrap items-center gap-x-2 gap-y-1 text-sm', $dark ? 'text-zinc-300' : 'text-zinc-600']) }}>
    ...
    <span><span class="font-semibold {{ $dark ? 'text-white' : 'text-zinc-900' }}">{{ number_format($google['rating'], 1) }}</span> on Google</span>
    @if ($since)

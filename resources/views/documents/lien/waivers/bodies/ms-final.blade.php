{{--
    Mississippi statutory form: Waiver and Release Upon Final Payment,
    Miss. Code Ann. § 85-7-433(2). A waiver requested in exchange for or to
    induce final payment "shall substantially follow" this form
    (§ 85-7-419(3)). The statute prints the form entirely in capital letters;
    statutory text is reproduced verbatim (in caps) with blanks bound to
    payload fields. Statutory element order preserved: body, signature, notary
    jurat, then the mandatory 60-day NOTICE, which must appear on the face of
    the form or the waiver is "unenforceable and invalid" under § 85-7-419.
    Unlike the interim form there is no through-date or retainage exception:
    this releases ALL lien and labor/material bond rights on account of labor
    or materials furnished to said contractor for the property. Must be sworn
    before a notary (e-sign disabled for MS).

    Print quirk in the statute: both the enrolled SB 2622 and the codified
    § 85-7-433(2) print a SECOND identical signature block and notary jurat
    after the NOTICE on this final form only (the interim form has one). It is
    an obvious duplication artifact; we render a single execution block, as
    the "shall substantially follow" standard (§ 85-7-419(3)) permits. Do not
    "fix" the NOTICE placement based on that stray block.
--}}
<div class="waiver-title">{{ $waiver['form']['title'] }}</div>
<div class="waiver-statute">State of Mississippi: Statutory form, Miss. Code Ann. &sect; 85-7-433(2)</div>
...
<div class="waiver-body">
    ...
    <p>
        ...
        DESCRIBED PROPERTY OR ANY RIGHTS AGAINST ANY LABOR AND/OR MATERIAL BOND ON ACCOUNT OF LABOR OR
        MATERIALS, OR BOTH, FURNISHED BY THE UNDERSIGNED TO OR ON ACCOUNT OF SAID CONTRACTOR FOR SAID
        PROPERTY.
    </p>
...
<p class="waiver-foot">
    Statutory form per Miss. Code Ann. &sect; 85-7-433(2); a waiver executed in exchange for or to induce
    final payment must substantially follow this form (&sect; 85-7-419(3)). The waiver is binding subject
    only to payment in full, and the amount is conclusively deemed paid sixty (60) days after the date of
    execution unless an Affidavit of Nonpayment (&sect; 85-7-433(3)) is filed before the period expires
    (&sect; 85-7-419(5)(b)). Form LW-MS-CF v{{ $waiver['form']['template_version'] }}.
</p>

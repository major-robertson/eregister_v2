{{--
    Utah statutory form: Utah Conditional Waiver and Release Upon Progress
    Payment, Utah Code § 38-1a-802(4)(b). A waiver meets the statute if it is
    in substantially this form (§ 38-1a-802(4)(a)); statutory text is
    reproduced verbatim with blanks bound to payload fields. Effective only
    once the undersigned endorses a check in the referenced Payment Amount and
    the check is paid by the depository institution on which it is drawn;
    § 38-1a-802(3) voids the waiver if the check fails to clear. The statute
    prescribes its own header fields, so the _identification-fields partial
    is skipped.
--}}
<div class="waiver-title">{{ $waiver['form']['title'] }}</div>
<div class="waiver-statute">State of Utah: Statutory form, Utah Code &sect; 38-1a-802(4)(b)</div>
...
<table class="waiver-fields">
    ...
    <tr>
        ...
        <td class="value">{{ !empty($waiver['through_date']) ? 'Through '.$waiver['through_date'] : '' }}&nbsp;</td>
    </tr>
</table>
...
<div class="waiver-body">
    ...
    <p>
        This waiver and release applies to a progress payment for the work, materials, equipment, or a
        combination of work, materials, and equipment furnished by the undersigned to the Property or to
        the Undersigned's Customer which are the subject of the Invoice or Payment Application, but only
        to the extent of the Payment Amount. This waiver and release does not apply to any retention
        withheld; any items, modifications, or changes pending approval; disputed items and claims; or
        items furnished or invoiced after the Payment Period.
    </p>
    ...
    <p>
        ...
        undersigned receives from this progress payment promptly to pay in full all the undersigned's
...
<p class="waiver-foot">
    Statutory form per Utah Code &sect; 38-1a-802(4)(b); substantial compliance permitted by
    &sect; 38-1a-802(4)(a). Enforceable only if the claimant receives payment of the amount identified,
    and for a progress payment only to the extent of the payment (&sect; 38-1a-802(2)(b)). If the check
    fails to clear the depository institution on which it is drawn for any reason, this waiver and
    release is void (&sect; 38-1a-802(3)). Form LW-UT-CP v{{ $waiver['form']['template_version'] }}.
</p>

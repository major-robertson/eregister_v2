<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <flux:heading size="xl">Formations Board</flux:heading>
            <flux:text class="mt-1">
                @if ($showAll)
                    Showing every status. Use the active board to focus on work that needs your attention.
                @else
                    Manage and process LLC formations across all businesses.
                @endif
            </flux:text>
        </div>
        @if ($showAll)
            <flux:button :href="route('admin.formations.board')" variant="subtle" icon="arrow-left" wire:navigate>
                Back to Active Board
            </flux:button>
        @else
            <flux:button :href="route('admin.formations.board-all')" variant="subtle" icon="view-columns" wire:navigate>
                View All Statuses
            </flux:button>
        @endif
    </div>
                ...
                >
                    @foreach ($columnCards as $card)
                        ...
                        <x-admin.kanban.card-frame
                            :href="route('admin.formations.states.show', $card)"
                        ...
                        >
                            <div class="flex items-start justify-between gap-2">
                                ...
                                <flux:badge color="indigo" size="sm">{{ $card->state_code }}</flux:badge>
                            </div>

<div class="w-full max-w-lg">
    {{-- Progress dots: 4 for continuous flow, 2 for standalone --}}
    @php $totalUnifiedSteps = $this->isContinuousFlow ? 4 : 2; $unifiedStep = $step + $stepOffset; @endphp
    <div class="mb-16 flex justify-center gap-2">
        @for ($i = 1; $i <= $totalUnifiedSteps; $i++)
            <div @class([
                'h-2 w-2 rounded-full transition-colors',
                'bg-primary' => $i <= $unifiedStep,
                'bg-zinc-300 dark:bg-zinc-600' => $i > $unifiedStep,
            ])></div>
        @endfor
    </div>
